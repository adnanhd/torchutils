...
class MetricType(FunctionalType):
    ...
    def field_validator(cls, field_type, info):
        if isinstance(field_type, torch.jit.ScriptFunction):
            def foo(batch_output: torch.Tensor, batch_target: torch.Tensor)  -> typing.Dict[str, float]:
                return field_type(batch_output=batch_output, batch_target=batch_target)
            foo.__name__ = field_type.name
            return foo
        if ray is not None and isinstance(field_type, ray.remote_function.RemoteFunction):
    ...
    def field_signature_validator(cls, field_type, info):
        ...
        if signature.return_annotation == float:
            ...
            raise AssertionError('return type mismatch in field_signature_validator')
